...
export const SellYourCarBanner: React.FC<SellYourCarBannerProps> = ({ setTab, lang }) => {
  ...
  return (
    <section className="w-full bg-[var(--color-bg-secondary)] py-16 px-4 sm:px-6 lg:px-8 border-t border-b border-[var(--color-border-main)] relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="bg-[var(--color-bg-primary)] border border-[var(--color-border-main)] rounded-3xl p-8 sm:p-12 shadow-2xl relative overflow-hidden">
          
          <div className="grid lg:grid-cols-12 gap-8 items-center">
            
            {/* Left Content */}
            <div className="lg:col-span-8 space-y-6">
              <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-[var(--color-accent-subtle)] border border-[var(--color-accent-main)]/30 text-[var(--color-accent-main)] text-xs font-mono font-bold uppercase tracking-wider">
                <Sparkles size={14} />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
                <div className="flex items-center gap-2.5 text-xs text-[var(--color-text-main)] font-medium">
                  <div className="w-6 h-6 rounded-full bg-[var(--color-accent-subtle)] text-[var(--color-accent-main)] flex items-center justify-center shrink-0">
                    <Clock size={12} />
                  </div>
              ...
              <div className="flex flex-wrap items-center gap-3 pt-2">
                <button
                  type="button"
                  onClick={() => setTab('sell')}
                  className="btn-gold-primary text-xs"
                >
                  <PlusCircle size={16} />
            ...
            <div className="lg:col-span-4 flex justify-center">
              <div className="w-full max-w-sm p-6 rounded-2xl bg-[var(--color-bg-secondary)] border border-[var(--color-border-main)] space-y-4 shadow-inner">
                <div className="flex items-center justify-between pb-3 border-b border-[var(--color-border-main)]">
                  ...
                  <span className="px-2 py-0.5 rounded-md bg-[var(--color-accent-subtle)] text-[var(--color-accent-main)] text-[10px] font-mono font-bold">2 Pathways</span>
                </div>

                <div className="space-y-3">
                  <div 
                    onClick={() => setTab('sell')}
                    className="p-3 rounded-xl bg-[var(--color-bg-primary)] border border-[var(--color-border-main)] hover:border-[var(--color-accent-main)]/50 transition-all cursor-pointer"
                  >
                    <div className="flex items-center justify-between">
                      <h4 className="text-xs font-bold text-[var(--color-text-main)]">Self-Managed Ad</h4>
                      <span className="text-[10px] font-mono font-bold text-[var(--color-accent-main)]">FREE</span>
                    </div>
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[10px] font-mono font-bold text-[var(--color-accent-main)]">CONCIERGE</span>
                    </div>
