...
export default function DealerStorefrontView({
  ...
  const socialMedia = dealer.socials || {
    ...
    linkedin: dealer.socials?.linkedin || '',
  ...
  const [activeTab, setActiveTab] = useState<string>(() => {
    if (typeof window !== 'undefined' && (sessionStorage.getItem('openHQTab') === 'true' || window.location.hash === '#hq')) {
      sessionStorage.removeItem('openHQTab');
      return 'hq';
    }
    return 'home';
  });
  ...
  useEffect(() => {
    if (sessionStorage.getItem('openHQTab') === 'true' || window.location.hash === '#hq') {
      setActiveTab('hq');
      sessionStorage.removeItem('openHQTab');
    }
  }, []);
  ...
  // Showroom Header Image Carousel state & array
  const showroomImages = useMemo(() => {
    const imgs: string[] = [];
    if (dealer.coverImage) imgs.push(dealer.coverImage);
    if (dealer.gallery && dealer.gallery.length > 0) {
      imgs.push(...dealer.gallery);
    }
    // High-end physical showroom trust photos
    imgs.push(
  ...
  const [currentHeaderImgIdx, setCurrentHeaderImgIdx] = useState(0);

  useEffect(() => {
    const timer = setInterval(() => {
      setCurrentHeaderImgIdx(prev => (prev + 1) % showroomImages.length);
    }, 5000);
  ...
  // Inline Logo & Cover upload state
  const [liveLogo, setLiveLogo] = useState<string>(dealer.logo || dealer.logoUrl || dealer.avatarUrl || '');
  const [liveCover, setLiveCover] = useState<string>(dealer.coverImage || '');
  const [logoUploading, setLogoUploading] = useState(false);
  const [coverUploading, setCoverUploading] = useState(false);

  useEffect(() => {
    setLiveLogo(dealer.logo || dealer.logoUrl || dealer.avatarUrl || '');
    setLiveCover(dealer.coverImage || '');
  }, [dealer]);

  const handleInlineLogoUpload = async (file: File) => {
    ...
    try {
      ...
      if (watermarkedDataUrl) {
        setLiveLogo(watermarkedDataUrl);
      } else {
        const reader = new FileReader();
        reader.onloadend = () => {
          if (reader.result) setLiveLogo(reader.result as string);
        };
        reader.readAsDataURL(file);
      }

      try {
        const res = await uploadToCloudinary(finalFile, {
          compress: true,
          resourceType: 'image',
          folder: 'bazar360/dealers/logos'
        });
        if (res && res.secure_url) {
          setLiveLogo(res.secure_url);
        ...
        showToast('✓ Showroom Logo updated & saved to Firestore!');
      } catch (cloudErr) {
        showToast('✓ Logo updated locally!');
      }
    } catch (err: any) {
      showToast('Failed to update logo: ' + (err.message || err));
    } finally {
      setLogoUploading(false);
    }
  };

  const handleInlineCoverUpload = async (file: File) => {
    ...
    try {
      ...
      if (watermarkedDataUrl) {
        setLiveCover(watermarkedDataUrl);
      } else {
        const reader = new FileReader();
        reader.onloadend = () => {
          if (reader.result) setLiveCover(reader.result as string);
        };
        reader.readAsDataURL(file);
      }
      ...
      try {
        ...
        showToast('✓ Showroom Cover image updated & saved!');
      } catch (cloudErr) {
        showToast('✓ Cover image updated locally!');
      }
    } catch (err: any) {
      showToast('Failed to update cover image: ' + (err.message || err));
    } finally {
  ...
  const isSuperOrAdminEmail = ['amjid.bisconni@gmail.com', 'khattakghani94@gmail.com', 'mazharsouls@gmail.com'].includes(currentUser?.email?.toLowerCase() || '');
  const hasOfficialRights = currentUser?.role === 'Admin' || 
  ...
  return (
    <div className={`space-y-6 pb-28 relative min-h-screen bg-[var(--color-bg-primary)] transition-colors duration-500`}>
      ...
      <section className={`bg-white border border-[var(--color-border-main)] rounded-3xl overflow-hidden relative shadow-[var(--shadow-bento)] select-none`}>
        ...
        {/* Interactive Showroom Header Image Carousel */}
        <div className="h-44 md:h-64 relative overflow-hidden bg-slate-950 group">
          {showroomImages.map((imgUrl, idx) => (
            <div 
              key={idx}
              className={`absolute inset-0 transition-opacity duration-1000 ${idx === currentHeaderImgIdx ? 'opacity-100 z-10' : 'opacity-0 z-0'}`}
            >
              <img 
                src={imgUrl} 
                alt={`${dealer.name} physical showroom ${idx + 1}`}
                className="w-full h-full object-cover opacity-90 object-center scale-105 transition-transform duration-[7000ms]"
          ...
          <div className="absolute top-0 left-0 right-0 h-[4px] bg-[var(--color-accent-main)] z-20"></div>
          ...
          <div className="absolute bottom-4 left-6 z-30 flex items-center gap-2">
            <button
              onClick={() => setCurrentHeaderImgIdx((prev) => (prev - 1 + showroomImages.length) % showroomImages.length)}
              className="w-7 h-7 rounded-full bg-black/60 hover:bg-black text-white flex items-center justify-center text-xs backdrop-blur-md transition-all border border-white/20 cursor-pointer shadow-lg"
            >
              ‹
            </button>
            <div className="flex items-center gap-1.5 px-2.5 py-1 bg-black/50 backdrop-blur-md rounded-full border border-white/10">
              {showroomImages.map((_, i) => (
                <button
                  key={i}
                  onClick={() => setCurrentHeaderImgIdx(i)}
                  className={`h-1.5 rounded-full transition-all ${i === currentHeaderImgIdx ? 'w-5 bg-orange-500' : 'w-1.5 bg-white/40'}`}
                />
              ))}
            </div>
            <button
              onClick={() => setCurrentHeaderImgIdx((prev) => (prev + 1) % showroomImages.length)}
              className="w-7 h-7 rounded-full bg-black/60 hover:bg-black text-white flex items-center justify-center text-xs backdrop-blur-md transition-all border border-white/20 cursor-pointer shadow-lg"
            >
              ›
            </button>
            <span className="text-[10px] font-mono text-white/90 font-bold ml-2 hidden sm:inline bg-black/40 px-3 py-1 rounded-full border border-white/10 shadow-sm">
              Showroom Tour {currentHeaderImgIdx + 1} / {showroomImages.length}
            </span>
          </div>
          
          {/* Top sharing & editing CTAs in the banner */}
          <div className="absolute top-4 right-4 flex items-center gap-2 z-10">
            {hasOfficialRights && (
              <label className="px-3.5 py-2 bg-slate-950/80 hover:bg-slate-950 text-amber-400 rounded-xl text-[10px] tracking-wider uppercase font-black border border-amber-500/30 flex items-center gap-1.5 transition-all shadow-md cursor-pointer">
                {coverUploading ? (
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (file) handleInlineCoverUpload(file);
                  }}
                />
              </label>
            )}
        ...
        <div className="px-6 pb-6 relative -mt-10 flex flex-col md:flex-row items-center md:items-end gap-5">
          ...
          <div className="w-24 h-24 md:w-32 md:h-32 rounded-full border-4 border-white overflow-hidden bg-white flex items-center justify-center shadow-lg relative z-10 shrink-0 group">
            {(liveLogo || dealer.logo || dealer.avatarUrl) ? (
              <img
                src={liveLogo || dealer.logo || dealer.avatarUrl}
                alt={dealer.name}
            ...
            {hasOfficialRights && (
              <label className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center text-white cursor-pointer rounded-full p-1 text-center z-20">
                {logoUploading ? (
                  <Loader2 className="animate-spin text-orange-400" size={22} />
                ) : (
                  <>
                    <Camera size={20} className="text-orange-400 mb-0.5" />
                    <span className="text-[9px] font-black uppercase tracking-wider leading-none">Edit Logo</span>
                  </>
                )}
                <input
                  type="file"
                  ...
                  onChange={(e) => {
                    const file = e.target.files?.[0];
                    if (file) handleInlineLogoUpload(file);
                  }}
                />
              </label>
            )}
          ...
          <div className="flex-grow text-center md:text-left space-y-2 mt-4 md:mt-0">
            ...
            {/* Strict Socials Hiding Rule: section is completely hidden if no links are provided. */}
            {(socialMedia.facebook || socialMedia.instagram || socialMedia.linkedin || socialMedia.whatsapp || socialMedia.tiktok || socialMedia.website) && (
              <div className="flex flex-wrap gap-2 pt-1 justify-center md:justify-start items-center">
                ...
                {socialMedia.website && (
                  <a
                    ...
                    title="Website"
                    className="w-8 h-8 rounded-lg bg-[var(--color-bg-primary)] border border-[var(--color-border-main)] hover:bg-[var(--color-accent-main)]/10 text-[var(--color-text-main)] hover:text-[var(--color-accent-main)] flex items-center justify-center transition-all duration-300 transform hover:scale-110 cursor-pointer shadow-sm hover:shadow-md"
                  >
                    <Globe size={14} />
                  </a>
                )}
                {socialMedia.whatsapp && (
                  <a
                    ...
                    rel="noopener noreferrer"
                    title="WhatsApp"
                    className="w-8 h-8 rounded-lg bg-[var(--color-bg-primary)] border border-[var(--color-border-main)] hover:bg-[#25D366]/15 text-[var(--color-text-main)] hover:text-[#25D366] hover:border-[#25D366]/40 flex items-center justify-center transition-all duration-300 transform hover:scale-110 cursor-pointer shadow-sm hover:shadow-md"
                  >
                ...
                {socialMedia.facebook && (
                  <a
                    ...
                    rel="noopener noreferrer"
                    title="Facebook"
                    className="w-8 h-8 rounded-lg bg-[var(--color-bg-primary)] border border-[var(--color-border-main)] hover:bg-[#1877F2]/15 text-[var(--color-text-main)] hover:text-[#1877F2] hover:border-[#1877F2]/40 flex items-center justify-center transition-all duration-300 transform hover:scale-110 cursor-pointer shadow-sm hover:shadow-md"
                  >
                    <Facebook size={14} />
                  </a>
                )}
                {socialMedia.instagram && (
                  <a
                    ...
                    rel="noopener noreferrer"
                    title="Instagram"
                    className="w-8 h-8 rounded-lg bg-[var(--color-bg-primary)] border border-[var(--color-border-main)] hover:bg-[#E4405F]/15 text-[var(--color-text-main)] hover:text-[#E4405F] hover:border-[#E4405F]/40 flex items-center justify-center transition-all duration-300 transform hover:scale-110 cursor-pointer shadow-sm hover:shadow-md"
                  >
                    <Instagram size={14} />
                  </a>
                )}
                {socialMedia.linkedin && (
                  <a
                    ...
                    rel="noopener noreferrer"
                    title="LinkedIn"
                    className="w-8 h-8 rounded-lg bg-[var(--color-bg-primary)] border border-[var(--color-border-main)] hover:bg-[#0A66C2]/15 text-[var(--color-text-main)] hover:text-[#0A66C2] hover:border-[#0A66C2]/40 flex items-center justify-center transition-all duration-300 transform hover:scale-110 cursor-pointer shadow-sm hover:shadow-md"
                  >
                    <Linkedin size={14} />
                  </a>
                )}
                {socialMedia.tiktok && (
                  <a
                    ...
                    rel="noopener noreferrer"
                    title="TikTok"
                    className="w-8 h-8 rounded-lg bg-[var(--color-bg-primary)] border border-[var(--color-border-main)] hover:bg-white/15 text-[var(--color-text-main)] hover:text-white hover:border-white/40 flex items-center justify-center transition-all duration-300 transform hover:scale-110 cursor-pointer shadow-sm hover:shadow-md"
                  >
                    <Link size={14} />
                  </a>
                )}
