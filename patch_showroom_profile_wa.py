with open('src/components/ShowroomProfile.tsx', 'r') as f:
    content = f.read()
...
replacement = """        <a 
          ...
          className="fixed bottom-24 right-6 md:right-8 z-50 group bg-[#25D366] hover:bg-[#20bd5a] text-white rounded-full p-4 shadow-2xl flex items-center justify-center cursor-pointer border border-[#25D366]/50 shadow-[#25D366]/30 transition-all duration-300 w-14 h-14 md:w-auto md:h-auto md:px-5 md:py-3.5 md:rounded-2xl hover:scale-[1.02] active:scale-95"
...
if start != -1:
    ...
    if end != -1:
        ...
        with open('src/components/ShowroomProfile.tsx', 'w') as f:
            f.write(content)
        print("ShowroomProfile Patched via manual split!")
else:
    # also try single line `<a href={`https://wa.me/`
    start = content.find('<a href={`https://wa.me/')
    if start != -1:
        end = content.find('</a>', start) + 4
        if end != -1:
            content = content[:start] + replacement + content[end:]
            with open('src/components/ShowroomProfile.tsx', 'w') as f:
                f.write(content)
            print("ShowroomProfile Patched via manual split (single line)!")
